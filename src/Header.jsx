...
const Header = () => {
  ...
  const menuDebugStartRef = useRef(0);

  const logMenuDebug = (label, extra = {}) => {
    const now = performance.now();
    const sinceClick = menuDebugStartRef.current ? (now - menuDebugStartRef.current).toFixed(1) : null;

    console.log('[MENU DEBUG]', label, {
      t: now.toFixed(1),
      sinceClick,
  ...
  useEffect(() => {
    ...
    logMenuDebug('visual effect entered');

    if (menuState === 'open') {
      if (isMobile) {
        logMenuDebug('visual effect set true immediately');
        setMenuVisualActive(true);
      } else {
        logMenuDebug('visual effect scheduling true', { delay: 80 });
        timeout = setTimeout(() => {
          logMenuDebug('visual effect set true from timeout');
          setMenuVisualActive(true);
        }, 80);
      }
    } else if (menuState === 'closing') {
      if (isMobile) {
        logMenuDebug('visual effect set false immediately');
        setMenuVisualActive(false);
      } else {
        logMenuDebug('visual effect scheduling false', { delay: 350 });
        timeout = setTimeout(() => {
          logMenuDebug('visual effect set false from timeout');
          setMenuVisualActive(false);
        }, 350);
      }
    } else if (!isMenuOpen) {
      logMenuDebug('visual effect set false from fallback');
      setMenuVisualActive(false);
    }

    return () => {
      logMenuDebug('visual effect cleanup');
  ...
  useEffect(() => {
    logMenuDebug('menuState changed');
  }, [menuState]);

  useEffect(() => {
    logMenuDebug('isMenuOpen changed');
  }, [isMenuOpen]);

  useEffect(() => {
    logMenuDebug('menuVisualActive changed');
  }, [menuVisualActive]);

  useEffect(() => {
    logMenuDebug('home/mobile/path changed');
  }, [isHomePage, isMobile, location.pathname]);

  useEffect(() => {
    logMenuDebug('hideHeader changed');
  }, [hideHeader]);

  useEffect(() => {
    logMenuDebug('delayTransparentHeader changed');
  }, [delayTransparentHeader]);

  useEffect(() => {
    logMenuDebug('isScrolled changed');
  }, [isScrolled]);
  ...
  useEffect(() => {
    requestAnimationFrame(() => {
      ...
      console.log('[MENU DEBUG] DOM snapshot', {
        pathname: location.pathname,
  ...
  }, [menuState, isMenuOpen, menuVisualActive, hideHeader, isHomePage, isMobile, location.pathname]);

  const handleToggleMenu = () => {
    menuDebugStartRef.current = performance.now();
    logMenuDebug('burger click before toggle');
    toggleMenu();
    logMenuDebug('burger click after toggle call');
  ...
  return (
    <>
      ...
      <div
        className={`mobile-menu${menuState === 'open' ? ' active' : ''}${menuState === 'closing' ? ' closing' : ''}${hideHeader ? ' hide-header' : ''}`}
        onTransitionStart={(e) => {
          console.log('[MENU DEBUG] transition start', {
            propertyName: e.propertyName,
            targetClassName: e.target.className
          });
        }}
        onTransitionEnd={(e) => {
          console.log('[MENU DEBUG] transition end', {
            propertyName: e.propertyName,
            targetClassName: e.target.className
          });
        }}
