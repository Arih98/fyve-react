...
const Header = () => {
  ...
  const headerDebugStartRef = useRef(0);
  ...
  const logHeaderDebug = (label, extra = {}) => {
    const now = performance.now();
    const sinceClick = headerDebugStartRef.current ? (now - headerDebugStartRef.current).toFixed(1) : null;

    const headerEl = headerRef.current;
    const menuEl = document.querySelector('.mobile-menu');
    const burgerEl = document.querySelector('.a-burger');

    const headerStyles = headerEl ? window.getComputedStyle(headerEl) : null;
    const burgerStyles = burgerEl ? window.getComputedStyle(burgerEl) : null;

    console.log('[HEADER DEBUG]', label, {
      t: now.toFixed(1),
      sinceClick,
      ...
      headerClassName: headerEl?.className || null,
      menuClassName: menuEl?.className || null,
      burgerClassName: burgerEl?.className || null,
      headerBackgroundColor: headerStyles?.backgroundColor || null,
      headerTransition: headerStyles?.transition || null,
      headerTransform: headerStyles?.transform || null,
      headerOpacity: headerStyles?.opacity || null,
      burgerColor: burgerStyles?.color || null,
      burgerTransition: burgerStyles?.transition || null,
      ...extra
    });
  };
  ...
  useEffect(() => {
    ...
    logHeaderDebug('menu visual effect entered');

    if (menuState === 'open') {
      logHeaderDebug('menu visual effect scheduling true', { delay: 80 });
      timeout = setTimeout(() => {
        logHeaderDebug('menu visual effect set true from timeout');
        setMenuVisualActive(true);
      }, 80);
    } else if (menuState === 'closing') {
      logHeaderDebug('menu visual effect scheduling false', { delay: 350 });
      timeout = setTimeout(() => {
        logHeaderDebug('menu visual effect set false from timeout');
        setMenuVisualActive(false);
      }, 350);
    } else if (!isMenuOpen) {
      logHeaderDebug('menu visual effect set false from fallback');
      setMenuVisualActive(false);
    }

    return () => {
      logHeaderDebug('menu visual effect cleanup');
  ...
  useEffect(() => {
    logHeaderDebug('menuState changed');
  }, [menuState]);

  useEffect(() => {
    logHeaderDebug('isMenuOpen changed');
  }, [isMenuOpen]);

  useEffect(() => {
    logHeaderDebug('menuVisualActive changed');
  }, [menuVisualActive]);

  useEffect(() => {
    logHeaderDebug('isScrolled changed');
  }, [isScrolled]);

  useEffect(() => {
    logHeaderDebug('hideHeader changed');
  }, [hideHeader]);

  useEffect(() => {
    logHeaderDebug('delayTransparentHeader changed');
  }, [delayTransparentHeader]);

  useEffect(() => {
    logHeaderDebug('useTransparentHomeHeader changed');
  }, [useTransparentHomeHeader]);

  useEffect(() => {
    logHeaderDebug('shouldBeTransparentHomeHeader changed');
  }, [shouldBeTransparentHomeHeader]);

  useEffect(() => {
    logHeaderDebug('home/mobile/path changed');
  }, [isHomePage, isMobile, location.pathname]);

  useEffect(() => {
    requestAnimationFrame(() => {
      logHeaderDebug('raf snapshot 1');
    });

    const raf2 = requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        logHeaderDebug('raf snapshot 2');
      });
    });
  ...
  const handleToggleMenu = () => {
    headerDebugStartRef.current = performance.now();
    logHeaderDebug('burger click before toggle');
    toggleMenu();
    logHeaderDebug('burger click after toggle call');
  ...
  const BurgerIcon = (
    <button
      type="button"
      className={`a-burger${menuState === 'open' || menuState === 'closing' ? ' menu-open' : ''}${menuState === 'open' ? ' circle-open' : ''}${isMenuOpen ? ' menu-active' : ''}${hideHeader ? ' hide-header' : ''}${useTransparentHomeHeader && !useCartHeaderVariant ? ' is-white' : ''}`}
      ref={burgerRef}
      onClick={handleToggleMenu}
      aria-expanded={isMenuOpen}
      aria-label={isMenuOpen ? 'Close menu' : 'Open menu'}
      onTransitionStart={(e) => {
        console.log('[HEADER DEBUG] burger transition start', {
      ...
      onTransitionEnd={(e) => {
        console.log('[HEADER DEBUG] burger transition end', {
          propertyName: e.propertyName,
          targetClassName: e.target.className,
          t: performance.now().toFixed(1)
        });
  ...
  return (
    <>
      <div
        ...
        onTransitionStart={(e) => {
          console.log('[HEADER DEBUG] header transition start', {
            propertyName: e.propertyName,
            targetClassName: e.target.className,
            t: performance.now().toFixed(1)
          });
          logHeaderDebug('header transition start', {
            propertyName: e.propertyName,
            targetClassName: e.target.className
          });
        }}
        onTransitionEnd={(e) => {
          console.log('[HEADER DEBUG] header transition end', {
            propertyName: e.propertyName,
            targetClassName: e.target.className,
            t: performance.now().toFixed(1)
          });
          logHeaderDebug('header transition end', {
            propertyName: e.propertyName,
            targetClassName: e.target.className
          });
        }}
          ...
          <>
            ...
            <div className="header-logo mobile-hide-logo">
              <img
                ...
                alt="FYVE Logo"
                onClick={() => navigate('/')}
                onLoad={() => logHeaderDebug('logo loaded', { logoSrc })}
              />
            </div>
            ...
            <div className="mobile-nav-icons">
              <button className="mobile-nav-icon" onClick={toggleSearch}>
                <img
                  src={searchIconSrc}
                  alt="Search"
                  onLoad={() => logHeaderDebug('search icon loaded', { searchIconSrc })}
                />
              </button>
              ...
              <button className="mobile-nav-icon" onClick={() => navigate('/account')}>
                <img
                  src={accountIconSrc}
                  alt="Account"
                  onLoad={() => logHeaderDebug('account icon loaded', { accountIconSrc })}
                />
              </button>
              ...
              <div className="header-bag-dropdown-wrap" ref={desktopCartRef}>
                ...
                >
                  <img
                    src={bagIconSrc}
                    alt="Bag"
                    onLoad={() => logHeaderDebug('bag icon loaded', { bagIconSrc })}
                  />
      ...
      <div
        ...
        onTransitionStart={(e) => {
          console.log('[HEADER DEBUG] menu transition start', {
            propertyName: e.propertyName,
            targetClassName: e.target.className,
            t: performance.now().toFixed(1)
          });
        ...
        onTransitionEnd={(e) => {
          console.log('[HEADER DEBUG] menu transition end', {
            propertyName: e.propertyName,
            targetClassName: e.target.className,
            t: performance.now().toFixed(1)
          });
